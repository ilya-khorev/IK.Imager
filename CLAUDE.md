...
- **`IK.Imager.Core.Abstractions` `ImageDetails` / `ImageDetailsWithThumbnails` / `ImageLookupResult`** — these were classes because the CDN decorators rewrote `Url` in place, including on thumbnails nested in a list. `IImageUrlBuilder` removed that reason: the url is now built correctly when the model is created and nothing patches it afterwards. Nothing stops these three from becoming records — they are still classes only because converting them is a change of its own.
