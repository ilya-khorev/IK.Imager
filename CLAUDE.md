...
**There is no overwrite.** A duplicate id is always a 409; replacing an image is delete-then-upload, which already removes the blobs and purges the CDN. Two things follow. First, the conflict has **two sources**: with no prefix the path is a function of the id, so Azure rejects it first - `AzureBlobImageRepository` translates that into `BlobAlreadyExistsException` rather than leaking `RequestFailedException` - while with a prefix the path is always new and the clash surfaces from `CreateMetadata` instead, at which point the blob just written is deleted before the 409 goes back. Second, **a blob on its own does not mean the id is taken**: deleting drops the metadata at once and the blobs off the bus a moment later, so re-uploading a just-deleted id finds a blob nothing owns. `ImageUploader` asks the metadata repository which case it is, and only then either fails or overwrites. Without that, replace-by-delete-then-upload would race.
