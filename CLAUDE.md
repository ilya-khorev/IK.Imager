...
Endpoint and schema descriptions come from the XML documentation of `IK.Imager.Api` and `IK.Imager.Api.Contract`, which the source generator inside `Microsoft.AspNetCore.OpenApi` bakes into the assembly at compile time — the XML files are no longer read at runtime, but `GenerateDocumentationFile` must stay on in both projects.

`IK.Imager.Api/OpenApi` replaces MicroElements.Swashbuckle.FluentValidation, which only plugs into Swashbuckle's schema generator: `FluentValidationRules` maps a property validator onto a schema (`NotEmpty`/`NotNull` → `required` plus `minLength`/`minItems`, `Length` → `minLength`/`maxLength`, `Matches` → `pattern`), and two transformers feed it. `FluentValidationSchemaTransformer` covers body-bound models; `FluentValidationOperationTransformer` covers form-bound ones, which ASP.NET Core flattens into one field per property before any schema transformer sees them, so they have to be matched back through `ApiParameterDescription.ModelMetadata`. Rules expressed as `Must(...)` predicates are invisible to both, exactly as they were to MicroElements.
