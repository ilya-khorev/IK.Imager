...
- **Positional**, for `ImageUploadOptions` - the four caller-chosen naming inputs of an upload, grouped so that `IImageUploader.Upload` does not take seven arguments. This is *not* the command object `ICommandHandler` was deleted for: nothing dispatches it, the uploader still takes plain arguments, and it exists only because four optional values that all shape one thing read better together. Note it is positional with defaults, so **construct it with named arguments** - `new ImageUploadOptions(Collection: x)` - since `new(x)` silently binds to `ImageId`.
...
- **`ImageMetadata`** — hand-writes `IEquatable` with a *sequence* comparison of `Thumbnails`. A generated record equality would silently downgrade that to reference equality on the `List<>`. It is also mutated in place before the upsert.
...
- `IK.Imager.Storage.AzureBlobs.Tests` and `IK.Imager.Storage.CosmosDb.Tests` — **require a running Docker daemon**, and nothing else. [Testcontainers](https://dotnet.testcontainers.org/) starts [Azurite](https://learn.microsoft.com/azure/storage/common/storage-use-azurite) and the [Linux Cosmos DB emulator](https://learn.microsoft.com/azure/cosmos-db/emulator-linux) on randomly mapped host ports and tears them down afterwards; both classes are tagged `[Trait("Category", "Integration")]`.
  ...
  - The Cosmos emulator serves plain HTTP in gateway mode and advertises its container-internal endpoint, so the fixture builds its own `CosmosClient` with `ConnectionMode.Gateway` + `LimitToEndpoint` + the module's URI-rewriting `HttpClientFactory`, and hands it to `ImageContainerFactory`. Both build their options through `ImageMetadataSerialization.CreateClientOptions()`, the same place the module takes them from, and add the emulator settings on top — see *Serialization*.
  ...
  - **Two things the vNext emulator does not do**, both verified against it rather than assumed. A **prefix (subpartition) partition key in `QueryRequestOptions` is silently ignored** - a query scoped to a tenant that does not even exist came back with every document in the container. It fails *open*, so the emulator can never prove tenant isolation: **any query using a prefix partition key must also carry an explicit `WHERE c.TenantId = @tenant`**, which works on the emulator and is free on real Cosmos, where the prefix already routes the query. Nothing in the repo needs such a query today - every read is a point read - but write the predicate the day one appears. And `DeleteAllItemsByPartitionKeyStreamAsync` answers `BadRequest`, so tenant offboarding cannot be exercised locally; it is a preview feature on real Cosmos too, so plan it as a fan-out delete.
- `IK.Imager.Api.Tests` — the API end to end, also **Docker-only**. `ImagerApiFixture` starts Azurite and the Cosmos emulator, then boots `IK.Imager.Api` itself in-process with `WebApplicationFactory<Program>` and drives it over HTTP. One host and one pair of containers for the whole assembly (`ImagerApiCollection`); a run takes ~20s.
  ...
  - The only registration overridden in the factory is `CosmosClient`, so the emulator options reach it — everything built on top (`IImageContainerFactory`, the repository, the health check) stays the production registration. It is the client rather than the factory precisely because the health check now resolves the same client.
  ...
  - Each test uses its own generated tenant (`NewTenantId()`). The host is shared and the tenant is the first level of the partition key, so that is what keeps one test's lookups blind to another's images.
...
A single ASP.NET Core service (`IK.Imager.Api`) that both serves the HTTP API **and** consumes its own integration events off Azure Service Bus. An earlier `IK.Imager.BackgroundService` microservice was folded into the API — `Scripts/DockerUpload.ps1` still refers to it. The architecture diagram lives in `README.md` as mermaid; `docs/` held a stale SVG that still showed the folded-in service, and is gone.

### Tenancy, and why there is no abstraction over identity

Every image belongs to a tenant. It is the first level of the Cosmos partition key, the scope an image id is unique in, and the first segment of every delivery url - so it is load-bearing in three places at once.

It reaches the service in the `X-Tenant-Id` header, and `IK.Imager.Api/Tenancy` is the whole of it: `TenantEndpointFilter` resolves and validates it once, a scoped `ITenantContext` holds it, and the endpoints pass the value into the core services as an ordinary argument. Nothing below the host knows where a tenant came from.

**`Tenancy:Source` picks the resolver, and the host owns the choice** - the same shape as `Cdn:Provider`, down to an unrecognised value throwing rather than falling back, because a typo in `Tenancy__Source` must not look like a service that is still isolating tenants. `HeaderTenantResolver` is today; `ClaimsTenantResolver` reads a claim named by `Tenancy:ClaimType`, which is configuration precisely because every identity provider names it differently (`tid` on Entra ID, `org_id` on Auth0).

**No authentication is built, and none should be invented.** `AddAuthentication` is already the provider-agnostic seam - every scheme, whatever the provider, ends at a `ClaimsPrincipal` - so `AddApiServices` takes an optional `Action<AuthenticationBuilder>` and that is the entire extension point. Do not add an `IIdentityProvider`, an `ITokenValidator`, an `ICurrentUser`, an authorization-policy wrapper, or per-provider modules like the CDN ones: a CDN purger is code only this repo can write, whereas an auth handler is already written by Microsoft or the vendor. When a provider appears the change is a hook, two config values and one `.RequireAuthorization()` on the `/images` group in `ImageEndpoints.cs`; core, storage, the url builder and the partition key are untouched.

**Until then the tenant is a data-partitioning parameter, not a security boundary.** Nothing verifies the header, so any caller that can reach the service can address any tenant. That is a sound trusted-subsystem arrangement *only* while the service is network-private - private endpoint, VNet, internal ingress. `/hc` and `/liveness` are outside the `/images` group and need no tenant at all, which is why the filter is an endpoint filter rather than middleware.
...
|---|---|---|
| `IImageUploader` | `Core/Upload/ImageUploader.cs` | `Upload(stream, tenantId, options, ct)`, `UploadByUrl(url, tenantId, options, ct)` |
| `IImageLookup` | `Core/Lookup/ImageLookup.cs` | `LookupByIds(ids, tenantId, ct)` |
| `IImageDeleter` | `Core/Delete/ImageDeleter.cs` | `DeleteMetadata(id, tenantId, ct) → bool`, `DeleteFiles(id, blobPath, thumbnailBlobPaths, ct)` |
| `IThumbnailGenerator` | `Core/Thumbnails/ThumbnailGenerator.cs` | `Generate(imageId, tenantId, ct)` - the widths come off the metadata, see *Identity and the url* |
...
- `DELETE /images/{imageId}` marks its request model `[AsParameters]`, binding `ImageId` from the route. The tenant comes from the header, so there is nothing else to bind. DELETE is one of the methods minimal APIs refuse to infer a **body** for, and that failure is an `InvalidOperationException` on the first request rather than a compile error — putting the id in the route avoids the shape entirely. Note an absent id then fails to match the route at all (a 404 from routing), so only a blank-but-present id reaches `DeleteImageRequestValidator`.
...
| `AddApiServices(Action<AuthenticationBuilder>?)` / `AddOpenApiDocumentation()` / `AddIntegrationEventMessaging(IConfiguration)` / `AddObservability(IConfiguration)` | `IK.Imager.Api/Extensions` | `Topics`, `Telemetry` |
| `AddTenancy(IConfiguration)` | `IK.Imager.Api/Tenancy` | `Tenancy` |
...
`AddApiServices` takes an optional `Action<AuthenticationBuilder>` for the same reason `AddImagerCore` takes one for its HTTP client: the host owns the policy. Passing nothing registers no scheme at all, and `Program.cs` only adds `UseAuthentication`/`UseAuthorization` when it finds an `IAuthenticationSchemeProvider` - calling them with no scheme registered throws rather than doing nothing.
...
Health checks stay in the host (`Extensions/ObservabilityExtensions.cs`) rather than moving into the storage modules — `AspNetCore.HealthChecks.*` is versioned independently and which endpoints get probed is an operational decision. They probe through the **`BlobServiceClient` and `CosmosClient` the storage modules register**, so a probe can never reach a different account, or authenticate differently, than the repositories do; only the database and container names still come from `IOptions<T>`. They used to build a client each from the connection string, which stopped being right the moment there was a second way to authenticate.
...
**There is no switch. The endpoint setting is the switch.** `AzureStorage:ServiceUri`, `CosmosDb:AccountEndpoint` and `ServiceBus:FullyQualifiedNamespace` each move their own client onto `DefaultAzureCredential`; leaving one empty keeps that client on its connection string. Nothing is passed down from the host, and no module reads anything outside its own section — the rule is one sentence, and there is no third setting whose value could disagree with the two that matter.

An earlier pass had a host-owned `Azure:Credential` switch threaded into every module as a `TokenCredential?` parameter. It bought a typo check and cost a setting, a host extension and a parameter on five registration methods — for a decision the endpoint already states. **Do not reintroduce it.**

**The endpoint wins when both it and a connection string are set**, deliberately: `appsettings.json` ships emulator connection strings as the local defaults, so requiring them to be blanked would make every keyless deployment set an empty environment variable — which some orchestrators drop rather than pass. Adding one variable is the whole migration.

**A module with neither setting throws at registration**, naming both. That is stricter than before, when a missing connection string surfaced from somewhere inside the SDK on the first call.

Each endpoint setting is named after its own SDK's vocabulary (`ServiceUri`, `AccountEndpoint`, `FullyQualifiedNamespace`) rather than made uniform — that is what makes it obvious which SDK argument the value becomes.

**Every module registers the credential with `TryAddSingleton<TokenCredential>`**, so the process shares one instance and one token cache however many modules need it, and a host that wants a credential of its own can register it before them. `DefaultAzureCredential`, not `ManagedIdentityCredential`: it is the managed identity in a deployment and the developer's own `az login` on a workstation, which is what makes a keyless setup runnable locally, and it picks up a user assigned identity from the standard `AZURE_CLIENT_ID` — so there is no client id setting of ours. It is also what `AzureFrontDoorCdnPurger` has always used; that module now registers the same shared credential instead of constructing its own inside the `ArmClient`.

Three things follow that are not obvious from the code alone, all verified against the services rather than assumed:

- **The Cosmos database and container are not created when `AccountEndpoint` is set.** Cosmos DB data plane RBAC covers reading and writing documents only; creating a database or a container is a control plane operation it cannot perform, so `CreateDatabaseIfNotExistsAsync` answers 403. `ImageContainerFactory` therefore takes a `provision` flag — false exactly when the endpoint is used — and takes a plain handle on the existing container instead. A keyless deployment provisions both alongside the account. Blob containers are the opposite case: creating one is inside Storage Blob Data Contributor, so that still happens on first use.
- **Service Bus needs Data Owner, not Data Sender/Receiver.** MassTransit builds its own topics and subscriptions through `ServiceBusAdministrationClient`, which is a management operation on the namespace.
- **Telemetry is the one flag, `Telemetry:EnableEntraIdAuthentication`, defaulting to false.** It is a flag rather than an endpoint because Application Insights needs its connection string either way — it carries the ingestion endpoint, not just the instrumentation key — so nothing about it could pick the authentication on its own. Off by default because authenticated ingestion also needs Monitoring Metrics Publisher, on a resource the storage and messaging role assignments say nothing about. `UseAzureMonitor` takes an `Action<AzureMonitorOptions>` with no way to reach the provider, so the credential goes on through a later `Configure<TokenCredential>`.

**`BlobServiceClient` and `CosmosClient` are now DI singletons**, registered by their modules and injected into `BlobContainerFactory` and `ImageContainerFactory`. Before, each factory built its own client from a connection string and the health checks built a third and a fourth. With two ways to authenticate that duplication would have been four places to keep in step, and a probe could have been reaching the account differently from the repository it is meant to be probing.
...
**Metadata documents go through System.Text.Json, and every `CosmosClient` must be built through `ImageMetadataSerialization.CreateClientOptions()`.** The SDK default serializer reads Newtonsoft attributes, which `ImageMetadata` no longer carries, so a client left on the default writes documents with no `id` at all. There are three construction sites — the module, `CosmosDbFixture` and `ImagerApiFixture` — and they all take the options from that one place precisely so a fixture cannot drift from production.

**The options are bare, and must stay bare.** System.Text.Json and the Newtonsoft serializer it replaced agree on everything this document needs: member names written unchanged, enums as numbers, nulls written, dates as ISO 8601. That was verified by serializing a fully-populated `ImageMetadata` with both and diffing — byte-identical — so documents written before the switch still read back. A `PropertyNamingPolicy` here would orphan every stored document *and* move `TenantId` out from under `/TenantId`, the first level of the partition key. `CreateMetadata_ValidMetadata_StoresDocumentWithUnchangedPropertyNames` reads the raw document back out of the emulator to pin exactly that; adding a camelCase policy fails ten tests in that suite.

**`Newtonsoft.Json` is still referenced, by `IK.Imager.Storage.CosmosDb` alone, and it is not ours.** `Microsoft.Azure.Cosmos.Client` has a direct assembly reference to Newtonsoft.Json 10.0.0.0 for its own internal types but declares *no* nuget dependency on it, leaving the version to the consumer — and ships a build target that fails the build when it finds no reference (`AzureCosmosDisableNewtonsoftJsonCheck` bypasses the check, which would only move the failure to runtime). So the package moved down to the one project whose SDK needs it, rather than disappearing. `IK.Imager.Storage.Abstractions` now has no package references at all, and `IK.Imager.Core.Abstractions` no longer picks Newtonsoft up transitively.
...
- **The tenant is there because it has to be.** Ids are unique per tenant while the blob containers are flat and global, so without it two tenants holding `sku-1234` would collide on one blob.
- **The collection goes before the prefix**, so that `acme/photos/` stays a usable blob prefix for listing or bulk deletion. A random segment in front of it would make every collection-scoped storage operation impossible.
- **Both middle segments are per-upload flags** (`IncludeCollectionInPath`, `AddUniquePrefix`), not configuration. A `Collection` on its own only labels the image.
- **The prefix obscures the url; it does not widen identity.** `sku-1234` twice in one tenant is a 409 whether or not a prefix was asked for, and `ImageIdentityTests` pins that - a caller may well assume otherwise.
...
First, **metadata decides, and it is asked first.** `ImageUploader.Upload` reads the metadata of the id before writing anything, so the ordinary duplicate is refused without a blob write, and every blob written past that point belongs to an id no image owns. It is a check, not the authority — `CreateMetadata` still is, because only Cosmos can settle two uploads racing on one id.

Second, **a blob on its own does not mean the id is taken**: deleting drops the metadata at once and the blobs off the bus a moment later, so re-uploading a just-deleted id finds a blob nothing owns. Given the check above, that is the only thing `BlobAlreadyExistsException` can now mean, so the uploader overwrites the orphan. (`AzureBlobImageRepository` translates Azure's 409 into that exception rather than leaking `RequestFailedException`.) Without this, replace-by-delete-then-upload would race.

Third, **the loser of a race leaves its blob behind on purpose.** When `CreateMetadata` conflicts, the upload gives up without deleting the blob it just wrote: with no unique prefix both uploads share a path, so that blob is very likely the one the winner's metadata now points at, and deleting it would leave an image resolving to nothing. Leaking one blob is the cheaper failure, and `ImageIdTakenWhileUploading` records the path at Warning so it can be found. An earlier version deleted unconditionally, which was only ever safe in the prefixed case.

**Thumbnail widths can come from the upload, and they travel on the metadata.** `Thumbnails:TargetWidth` is the default; `ThumbnailTargetWidths` on either upload request replaces it for one image. The chosen widths are stored on `ImageMetadata` rather than carried on `OriginalImageUploadedIntegrationEvent`, which is what keeps the event contract and `IThumbnailGenerator.Generate` unchanged - the generator reads them off the metadata it already fetches. It also means a job replayed later rebuilds the same thumbnails: the publisher's own comment describes republishing the event for an upload whose event was lost, and widths on the message would silently fall back to the configured ones there. The validators cap a request at 10 distinct positive widths - an operator sets the configured widths once, whereas this is per request and every width is one more resize. An empty list is **rejected**, not read as "no thumbnails": a caller wanting the configured widths omits the property, and guessing between the two readings would be a silent surprise.

Two things follow for the tests. `int[]` **does** bind from a repeated multipart form field into the `[FromForm]` model - verified against the running host, like the other binding notes above - so `ImagerApiClient.PostUpload` adds one form field per width. And AutoFixture now fills `ImageMetadata.ThumbnailTargetWidths`, which beats the configured widths, so any `ThumbnailGeneratorTests` case that is *about* configuration has to set it to null explicitly.

**Thumbnail paths are derived from the original's**, `{stem}_{width}.{extension}`, so they inherit the tenant, collection and prefix without being told any of them, and regeneration overwrites the previous set instead of orphaning it. That is also the only reason `IImageBlobRepository.UploadImage` has an `allowOverwrite` parameter - a redelivered thumbnail job would otherwise fail forever against `IfNoneMatch`.

### Validation is two-layered

FluentValidation validators live next to the feature they guard under `IK.Imager.Api/Features` and check the *request shape* (URL well-formed, the identifier charset below, `Collection` length 3–30, ≤200 image ids); they also surface into the OpenAPI document. FluentValidation 12 dropped built-in ASP.NET Core auto-validation, so `IK.Imager.Api/Validation/ValidationEndpointFilter.cs` runs the validator over the argument and short-circuits with a 400 `ValidationProblemDetails`. It is attached per endpoint with `.WithValidation<TRequest>()`, which also declares the 400 in the document — an endpoint that takes a validated model and forgets the call silently accepts anything, so add it alongside the `Map…` line. `IK.Imager.Core.Upload.ImageValidator` checks the *image itself* (format/size/dimensions/aspect ratio) against the `ImageLimitations` config section. **`ImageLimitations:SizeBytes.Max` is also enforced during the download** - see *Upload-by-url is the one place a caller picks the address* below. **`IImageInspector` is the only seam over reading and checking an image**, and `ImageUploader` never calls the validator itself: `ImageInspector.Inspect` reads the format and the size through the static `ImageFileReader` (all the ImageSharp code, no state, no dependencies), checks each through `ImageValidator`, logs the rejection and throws. `IImageInspector` and `IImageValidator` used to be two interfaces the uploader interleaved by hand — four calls and two null tests inside its own `Inspect` — which is what made its constructor nine parameters long. The two classes stay apart, because reading bytes and checking numbers against config change for different reasons, but only one of them reaches the uploader. Note the core services currently throw `ValidationException` on failure (there are `//todo`s about returning an error model instead); `IK.Imager.Api/ExceptionHandling/GlobalExceptionHandler.cs` maps it to a 400 and everything else to a 500 (with the full exception in `developerMessage` in Development). It is an `IExceptionHandler` registered on the pipeline rather than an MVC filter, because an endpoint filter cannot see an exception thrown by another filter; there is no developer exception page, since the MVC filter suppressed it for every action exception anyway.

**Both upload routes also bound their request body**, so an oversized one is refused as it arrives rather than after the whole of it has been buffered. `POST /images/upload` allows `ImageLimitations:SizeBytes.Max` plus 8 KB of multipart framing — `ImageValidator` only ever sees a stream that is already complete, which is the same reason `ImageDownloader` checks `Content-Length` first. `POST /images/upload-by-url` allows a flat 16 KB, because the image never travels through its body: a url and a handful of short fields, with roughly 8 KB left for the url alone. Minimal APIs have no `WithRequestSizeLimit()`, so it is a small `IRequestSizeLimitMetadata` record passed to `.WithMetadata()`, which routing then hands to `IHttpMaxRequestBodySizeFeature`. Two consequences. The multipart value is read from `IOptions<>` once at startup, because metadata is static — raising `SizeBytes.Max` on a running host moves what `ImageValidator` accepts but not what the endpoint refuses. And **only a real server implements that feature**: `Microsoft.AspNetCore.TestHost` does not, so the limit does nothing under `WebApplicationFactory`, and an integration test posting an oversized body would pass whatever the limit says. `UploadRequestSizeLimitTests` therefore asserts the metadata on the endpoint, not a 413.
...
`BeginScope` with a `Dictionary<string, object>` — never an interpolated string, which produces no properties. `ImageUploader.Upload` opens one on `ImageId`/`TenantId` as soon as the id is known (it cannot be earlier; a generated id does not exist yet), and each of the three consumers opens one so that `ThumbnailGenerator`, `ImageDeleter` and the CDN purgers inherit it. Trace context ties an upload to the consumer that thumbnails it seconds later, but only **when telemetry is configured** — see the `AddSource` note below — and even then a trace id does not say *which image* the lines are about, which is what an operator greps for. The scope is the half that works either way. `IncludeScopes` has to be on in two places: the console formatter and `OpenTelemetryLoggerOptions`.
...
`Tenancy:Source` defaults to `Header`; see *Tenancy, and why there is no abstraction over identity*. `AzureStorage:ServiceUri`, `CosmosDb:AccountEndpoint` and `ServiceBus:FullyQualifiedNamespace` are empty, and setting one moves that service off its connection string and onto `DefaultAzureCredential` - see *Authenticating to Azure*. `CosmosDb:ContainerId` is `imagemetadatacontainer-v2` - the suffix is deliberate, see *Storage model*.
